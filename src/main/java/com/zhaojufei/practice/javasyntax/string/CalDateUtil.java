...
public class CalDateUtil {
     ...
     * @return
     */
    public static List<String> getStrDayPriod(String start, String end) {
        Date beginDate = DateUtil.parseDate(start);
        Date endDate = DateUtil.parseDate(end);

        List<Date> dayPriod = getDayPriod(beginDate, endDate, true);
        List<String> dateList = Lists.newArrayList();

        for (Date date : dayPriod) {
     ...
     * @return 两日期之间的月数集合
     */
    public static List<Date> getMonthPeriod(Date beginDate, Date endDate) {
        ...
        result.add(formatDate(beginDate));

        long size = DateUtil.betweenMonth(begin, end, true);
        for (long i = 0; i < size; i++) {
            Date nextDate = DateUtil.offsetMonth(beginDate, (int) (i + 1));
            result.add(formatDate(nextDate));
        }
        return result;
    }

    public static void main(String[] args) {
        System.out.println(getStrDayPriod("2021-08-26", "2021-8-28"));
    }
