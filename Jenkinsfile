pipeline {
  ...
  stages {
    stage('Git Parameter') {
      steps {
        git branch: "${params.BRANCH}", url: 'git@gitlab.choicesoft.com.cn:choice-credit/choice-credit.git'
      }
    }
    stage('Build Project') {
      steps {
        script{
          ...
          sh '/bin/sh ./gradlew clean build -x test'
  ...
  environment {
    //修改appname
    appname = "choice-credit-${params.build_idc}"
    ...
    JAVA_OPTS = "-server -Xms768m -Xmx768m -Xmn512m -XX:MetaspaceSize=512m -XX:MaxMetaspaceSize=512m"
  }
  parameters {
    ...
    choice(name: 'build_env', choices: ['fat','dev','ttt','pre'], description: '将应用部署到以下环境')
